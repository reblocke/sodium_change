...
same model code when the site is hosted from `docs/`. Browser-facing calculation assembly now
lives in `src/sodium_uncertainty/calculator.py`; `docs/app.py` is only a small Pyodide entry shim
that exposes the package `compute_from_json` function. The docs package copy is generated by
`scripts/stage_docs_python.py` and should stay in sync with the core package.
...
## Serial-test interpretation layer
